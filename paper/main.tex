...
In this project, we investigate duplicate detection in a technical domain using the sentence-transformers/stackexchange-duplicates dataset from HuggingFace. We analyze how linguistic complexity affects model performance by comparing lexical baselines with transformer-based representations on questions of varying length and structure. We also ask whether methods that work on QQP remain effective for longer, domain-specific StackExchange posts.
...
Technical Q\&A sites such as StackExchange introduce additional complexity beyond what general-purpose datasets capture. Questions may contain multi-step explanations, configuration details, or extensive logs, and duplicates can be phrased in structurally different ways. These observations motivate us to compare a range of lexical and pretrained transformer approaches on technical-domain data and to evaluate whether techniques that work in general domains remain effective for technical questions.

Our study follows this direction by evaluating TF-IDF with Logistic Regression as a baseline, alongside several transformer families, including BERT, RoBERTa, DeBERTa, ModernBERT, and SBERT, on a reconstructed StackExchange duplicate question dataset, sentence-transformers/stackexchange-duplicates.

\section{Methods}
\subsection{Dataset Construction}
We construct our dataset from the publicly available sentence-transformers/stackexchange-duplicates dataset on HuggingFace, which provides positive duplicate pairs for three configurations: title pairs, body pairs, and post pairs. Because the original dataset contains only duplicate pairs, additional processing is required to generate corresponding non-duplicate examples and produce a balanced classification dataset.

For each configuration, we treat every duplicate pair as an undirected edge in a graph whose nodes represent questions. We then extract connected components to be duplicate clusters: all questions within a cluster are treated as duplicates. This prevents accidental label leakage, since duplicates from the same cluster must not appear across different splits of the dataset.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.1\linewidth]{ModelPerformance.png}
